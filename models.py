...
from services.algorithm import (
    TopicMemory, TopicState, Rating, CognitiveState, 
    ComprehensiveTopicScheduler, StudySession
)
...
class Topic(db.Model):
    ...
    mastery_level = db.Column(db.String(20), default='beginner', nullable=False)  # beginner/developing/proficient/advanced
    confidence_score = db.Column(db.Float, default=0.0, nullable=False)  # 0.0 to 1.0
    last_performance = db.Column(db.String(20), nullable=True)  # excellent/good/fair/poor/critical
    ...
    recent_performance_trend = db.Column(db.String(20), default='stable', nullable=False)  # improving/declining/stable
    estimated_next_success_rate = db.Column(db.Float, nullable=True)  # Algorithm prediction
    ...
    # Relationships
    review_sessions = db.relationship('ReviewSession', backref='topic', lazy='dynamic', 
                                     cascade='all, delete-orphan')
    exam_associations = db.relationship('ExamTopic', backref='topic', lazy='dynamic',
                                       cascade='all, delete-orphan')
    ...
    def to_algorithm_memory(self) -> TopicMemory:
        ...
        recent_sessions = self.review_sessions.order_by(
            ReviewSession.reviewed_at.desc()
        ).limit(10).all()
        
        recent_ratings = [Rating(session.rating) for session in recent_sessions]
        response_times = [session.response_time_seconds or 5.0 for session in recent_sessions 
                         if session.response_time_seconds]
        
        # Get exam dates
        exam_dates = [assoc.exam.exam_date for assoc in self.exam_associations 
                     if assoc.exam.exam_date >= date.today()]
        exam_weights = [assoc.importance_weight for assoc in self.exam_associations
                       if assoc.exam.exam_date >= date.today()]
        ...
        for i in range(1, len(sorted_sessions)):
            time_diff = (sorted_sessions[i].reviewed_at - sorted_sessions[i-1].reviewed_at).total_seconds()
    ...
    def update_from_algorithm_result(self, algorithm_result: Dict):
        ...
        self.successful_reviews = sum(1 for r in memory.recent_ratings if r >= Rating.GOOD)
        self.best_streak = max(self.best_streak, memory.streak)
        
        # Update performance indicators
        strength = algorithm_result.get('strength_analysis', {})
        self.confidence_score = strength.get('readiness_score', self.confidence_score)
        self.last_performance = strength.get('readiness_category', self.last_performance)
        self.estimated_next_success_rate = strength.get('exam_adjusted_retrievability', None)
    ...
    def _calculate_mastery_level(self, memory: TopicMemory, strength: Dict) -> str:
        ...
        if memory.state == TopicState.MATURE and maturity >= 0.8 and retrievability >= 0.85:
            return 'advanced'
        elif memory.state in [TopicState.REVIEW, TopicState.MATURE] and maturity >= 0.6:
            return 'proficient'
        elif memory.state in [TopicState.YOUNG, TopicState.REVIEW] and memory.streak >= 3:
            return 'developing'
        else:
            return 'beginner'
    ...
    def _calculate_performance_trend(self) -> str:
        ...
        recent_avg = statistics.mean([s.retention_percentage for s in recent_half 
                                    if s.retention_percentage is not None])
        older_avg = statistics.mean([s.retention_percentage for s in older_half 
                                   if s.retention_percentage is not None])
    ...
    def get_study_statistics(self) -> Dict:
        ...
        successful = self.review_sessions.filter(ReviewSession.rating >= ReviewRating.GOOD).count()
        ...
        return {
            ...
            'last_studied': self.last_reviewed_date.strftime('%B %d, %Y') if self.last_reviewed_date else 'Never'
        }
    
    def get_recent_performance(self, limit: int = 10) -> List[Dict]:
        """Get recent performance history for user display"""
        sessions = self.review_sessions.order_by(
            ReviewSession.reviewed_at.desc()
        ).limit(limit).all()
        ...
        for session in sessions:
            performance_data.append({
                ...
                'retention_rate': f"{session.retention_percentage}%" if session.retention_percentage else "N/A",
                'study_duration': f"{session.duration_minutes} min" if session.duration_minutes else "N/A",
                'response_time': f"{session.response_time_seconds:.1f}s" if session.response_time_seconds else "N/A",
    ...
    def _format_next_review_timing(self) -> str:
        """Format next review date in user-friendly way"""
        if not self.next_review_date:
            return "Not scheduled"
        
        now = now_ist()
        delta = ensure_timezone_aware(self.next_review_date) - now
        ...
        if delta.total_seconds() < 0:
            ...
            return f"Due {self.next_review_date.strftime('%B %d')}"
    ...
    def _identify_weak_areas(self) -> List[str]:
        ...
        if self.current_streak < 3:
            weak_areas.append("Consistency (frequent mistakes)")
    ...
    def _identify_strengths(self) -> List[str]:
        ...
        if self.current_streak >= 5:
            strengths.append(f"Excellent consistency ({self.current_streak} correct in a row)")
        
        if self.average_response_time and self.average_response_time < 3.0:
            strengths.append("Fast recall (quick response times)")
        
        if self.total_reviews > 20 and self.successful_reviews / self.total_reviews > 0.8:
...
class ReviewSession(db.Model):
    ...
    study_method = db.Column(db.String(100), nullable=True)  # flashcards, reading, practice, etc.
    ...
    def to_user_display(self) -> Dict:
        ...
        return {
            ...
            'retention_rate': f"{self.retention_percentage}%" if self.retention_percentage else "Not estimated",
            'time_spent': f"{self.duration_minutes} minutes" if self.duration_minutes else "Not recorded",
            'response_speed': self._get_response_speed_description(),
            'difficulty_felt': f"{self.difficulty_rating}/10" if self.difficulty_rating else "Not rated",
...
class Exam(db.Model):
    ...
    importance = db.Column(db.String(20), default='medium', nullable=False)  # low/medium/high/critical
    exam_type = db.Column(db.String(50), nullable=True)  # final, midterm, quiz, certification
    total_marks = db.Column(db.Integer, nullable=True)
    passing_marks = db.Column(db.Integer, nullable=True)
    
    # Preparation tracking
    preparation_status = db.Column(db.String(20), default='not_started', nullable=False)  # not_started/in_progress/well_prepared/review_ready
    ...
    estimated_performance = db.Column(db.String(20), nullable=True)  # excellent/good/fair/poor/critical
    ...
    # Relationships
    topic_associations = db.relationship('ExamTopic', backref='exam', lazy='dynamic',
                                        cascade='all, delete-orphan')
    study_plans = db.relationship('StudyPlan', backref='exam', lazy='dynamic',
                                 cascade='all, delete-orphan')
    ...
    def get_preparation_summary(self) -> Dict:
        ...
        for topic in topics:
            ...
            strength = scheduler.calculate_realistic_topic_strength(
                memory, exam_context={'overall_preparation': self.overall_readiness_score / 100}
            )
            ...
            topic_readiness.append({
                ...
                'confidence_range': f"{strength['confidence_interval']['lower']*100:.0f}%-{strength['confidence_interval']['upper']*100:.0f}%",
                'last_studied': topic.last_reviewed_date.strftime('%b %d') if topic.last_reviewed_date else 'Never',
        ...
        # Time management
        if days_remaining > 0:
            recommended_daily_hours = max(1.0, min(6.0, (self.estimated_study_hours_needed or 20) / days_remaining))
        else:
        ...
        return {
            ...
            'topic_breakdown': sorted(topic_readiness, key=lambda x: x['readiness_score']),
            'study_recommendations': {
                'daily_hours_needed': round(recommended_daily_hours, 1),
                'total_hours_remaining': round((self.estimated_study_hours_needed or 0) - self.actual_study_hours, 1),
                'priority_topics': [t['name'] for t in sorted(topic_readiness, key=lambda x: x['readiness_score'])[:3]],
    ...
    def _get_focus_recommendations(self, topic_readiness: List[Dict]) -> List[str]:
        ...
        if len(weak_topics) > 5:
            recommendations.append(f"Too many weak topics ({len(weak_topics)}) - consider prioritizing most important ones")
        elif weak_topics:
            recommendations.append(f"Focus on {len(weak_topics)} topics scoring below 60%")
        
        never_studied = [t for t in topic_readiness if t['last_studied'] == 'Never']
        if never_studied:
            recommendations.append(f"Start with {len(never_studied)} topics you haven't studied yet")
...
class ExamTopic(db.Model):
    ...
    importance_weight = db.Column(db.Float, default=1.0, nullable=False)  # How important for this exam
    expected_marks_percentage = db.Column(db.Float, nullable=True)  # What % of exam marks
    confidence_level = db.Column(db.String(20), default='medium', nullable=False)  # low/medium/high
...
class StudyPlan(db.Model):
    ...
    def get_today_plan(self) -> Dict:
        ...
        try:
            ...
            return {'message': 'Plan data corrupted', 'topics': [], 'estimated_hours': 0}
    ...
    def get_weekly_overview(self) -> Dict:
        ...
        try:
            ...
            for date_str in week_dates:
                day_plan = next((d for d in schedule if d.get('date') == date_str), None)
                if day_plan:
            ...
            return {
                ...
                'completion_rate': round((completed_days / 7) * 100, 1) if week_plan else 0
            }
        except (json.JSONDecodeError, KeyError):
...
class StudySession(db.Model):
    ...
    study_method = db.Column(db.String(100), nullable=True)  # reading, practice_problems, flashcards, etc.
    ...
    def get_session_summary(self) -> Dict:
        ...
        try:
            if self.topics_covered:
                ...
                topics_list = [Topic.query.get(tid).name for tid in topic_ids if Topic.query.get(tid)]
        ...
        return {
            ...
            'study_method': self.study_method or 'Not specified',
            'productivity': f"{self.productivity_rating}/10" if self.productivity_rating else "Not rated",
...
class LearningAnalytics(db.Model):
    ...
    consistency_score = db.Column(db.Float, nullable=True)  # How consistent study schedule was
    
    # Behavioral insights
    most_productive_time = db.Column(db.String(20), nullable=True)  # morning, afternoon, evening
    ...
    def generate_weekly_analytics(week_start_date: date) -> 'LearningAnalytics':
        ...
        week_reviews = ReviewSession.query.filter(
            ReviewSession.reviewed_at >= datetime.combine(week_start_date, datetime.min.time()),
            ReviewSession.reviewed_at <= datetime.combine(week_end, datetime.max.time())
        ).all()
        
        # Get all study sessions in this week
        week_studies = StudySession.query.filter(
            StudySession.started_at >= datetime.combine(week_start_date, datetime.min.time()),
            StudySession.started_at <= datetime.combine(week_end, datetime.max.time())
        ).all()
        
        # Calculate metrics
        total_reviews = len(week_reviews)
        successful_reviews = len([r for r in week_reviews if r.rating >= ReviewRating.GOOD])
        total_study_time = sum([s.duration_minutes or 0 for s in week_studies])
        
        # Calculate average retention
        retention_rates = [r.retention_percentage for r in week_reviews if r.retention_percentage is not None]
        avg_retention = statistics.mean(retention_rates) if retention_rates else None
        
        # Calculate response times
        response_times = [r.response_time_seconds for r in week_reviews if r.response_time_seconds is not None]
    ...
    def get_insights_summary(self) -> Dict:
        ...
        if success_rate >= 80:
            insights.append("Excellent performance! You're mastering topics consistently.")
        ...
        if self.total_study_time < 60:  # Less than 1 hour per day average
            ...
            insights.append("High study volume - make sure to take breaks to avoid burnout.")
        ...
        return {
            ...
            'average_retention': round(self.average_retention_rate, 1) if self.average_retention_rate else "N/A",
...
def initialize_database():
    ...
    db.engine.execute('CREATE INDEX IF NOT EXISTS idx_topics_next_review ON topics(next_review_date);')
    db.engine.execute('CREATE INDEX IF NOT EXISTS idx_topics_subject_mastery ON topics(subject, mastery_level);')
    db.engine.execute('CREATE INDEX IF NOT EXISTS idx_reviews_date_rating ON review_sessions(reviewed_at, rating);')
    db.engine.execute('CREATE INDEX IF NOT EXISTS idx_exams_date_status ON exams(exam_date, preparation_status);')
...
def get_due_topics(limit: int = 20) -> List[Topic]:
    ...
    return Topic.query.filter(
        Topic.next_review_date <= current_time
    ).order_by(Topic.next_review_date.asc()).limit(limit).all()
...
def get_learning_dashboard() -> Dict:
    ...
    recent_study_time = db.session.query(
        db.func.sum(StudySession.duration_minutes)
    ).filter(StudySession.started_at >= week_ago).scalar() or 0
    ...
    readiness_distribution = {'excellent': 0, 'good': 0, 'fair': 0, 'poor': 0, 'critical': 0}
    ...
    return {
        'due_for_review': {
            ...
            'topics': [{'name': t.name, 'subject': t.subject, 'overdue_by': (datetime.now() - t.next_review_date).days} 
                      for t in due_topics[:5]]
        },
        'upcoming_exams': {
            'count': len(upcoming_exams),
            'exams': [{'name': e.name, 'date': e.exam_date.strftime('%B %d'), 
        ...
        'knowledge_overview': {
            ...
            'avg_confidence': round(statistics.mean([t.confidence_score for t in all_topics]), 2) if all_topics else 0
        }
    }
