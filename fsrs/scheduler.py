...
from fsrs.topic import Topic, State
...
class Scheduler:
    ...
    Enables the reviewing and future scheduling of topics according to the FSRS algorithm.

    Attributes:
        parameters: The model weights of the FSRS scheduler.
        desired_retention: The desired retention rate of topics scheduled with the scheduler.
        learning_steps: Small time intervals that schedule topics in the Learning state.
        relearning_steps: Small time intervals that schedule topics in the Relearning state.
        maximum_interval: The maximum number of days a Review-state topic can be scheduled into the future.
    ...
    def get_topic_retrievability(
        self, topic: Topic, current_datetime: datetime | None = None
    ) -> float:
        """
        Calculates a Topic object's current retrievability for a given date and time.

        The retrievability of a topic is the predicted probability that the topic is correctly recalled at the provided datetime.

        Args:
            topic: The topic whose retrievability is to be calculated
            current_datetime: The current date and time

        Returns:
            The retrievability of the Topic object.
        """

        if topic.last_review is None:
            return 0

        if current_datetime is None:
            current_datetime = datetime.now(timezone.utc)

        elapsed_days = max(0, (current_datetime - topic.last_review).days)

        return (1 + self._FACTOR * elapsed_days / topic.stability) ** self._DECAY
    ...
    def review_topic(
    ...
    ) -> tuple[Topic, ReviewLog]:
        """
        Reviews a topic with a given rating at a given time for a specified duration.

        Args:
            topic: The topic being reviewed.
            rating: The chosen rating for the topic being reviewed.
        ...
        Returns:
            A tuple containing the updated, reviewed topic and its corresponding review log.
        ...
        days_since_last_review = (
            (review_datetime - topic.last_review).days if topic.last_review else None
        )

        match topic.state:
            case State.Learning:
                # update the topic's stability and difficulty
                if topic.stability is None and topic.difficulty is None:
                    topic.stability = self._initial_stability(rating=rating)
                    topic.difficulty = self._initial_difficulty(
                        rating=rating, clamp=True
                    )

                elif days_since_last_review is not None and days_since_last_review < 1:
                    topic.stability = self._short_term_stability(
                        stability=topic.stability, rating=rating
                    )
                    topic.difficulty = self._next_difficulty(
                        difficulty=topic.difficulty, rating=rating
                    )

                else:
                    topic.stability = self._next_stability(
                        difficulty=topic.difficulty,
                        stability=topic.stability,
                        retrievability=self.get_topic_retrievability(
                            topic,
                ...
                # calculate the topic's next interval
                ## first if-clause handles edge case where the Topic in the Learning state was previously
                ## scheduled with a Scheduler with more learning_steps than the current Scheduler
                if len(self.learning_steps) == 0 or (
                    topic.step >= len(self.learning_steps)
                    and rating in (Rating.Hard, Rating.Good, Rating.Easy)
                ):
                    topic.state = State.Review
                    topic.step = None

                    next_interval_days = self._next_interval(stability=topic.stability)
                    ...
                    match rating:
                        case Rating.Again:
                            topic.step = 0
                            next_interval = self.learning_steps[topic.step]

                        case Rating.Hard:
                            # topic step stays the same
                ...
                match rating:
                    case Rating.Again:
                        ...
                        if len(self.relearning_steps) == 0:
                            ...
                            next_interval = self.relearning_steps[topic.step]
            ...
            case State.Relearning:
                ...
                # calculate the topic's next interval
                ## first if-clause handles edge case where the Topic in the Relearning state was previously
                ## scheduled with a Scheduler with more relearning_steps than the current Scheduler
                if len(self.relearning_steps) == 0 or (
                    topic.step >= len(self.relearning_steps)
                    and rating in (Rating.Hard, Rating.Good, Rating.Easy)
                ):
                    topic.state = State.Review
                    topic.step = None
        ...
        if self.enable_fuzzing and topic.state == State.Review:
    ...
    def _get_fuzzed_interval(self, *, interval: timedelta) -> timedelta:
        ...
        For example, a topic that would've been due in 50 days, after fuzzing, might be due in 49, or 51 days.
